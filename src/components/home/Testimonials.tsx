      ...
      <div className='mt-32 mb-20'>
        <div className='flex items-center justify-start text-start'>
          <h2 className='text-[50px] font-extrabold leading-snug text-left'>
            <span className='text-orange-500 '>Quotes </span>
          ...
          <Carousel className='w-full'>
                ...
                <CarouselItem key={index}>
                  <div className='p-1'>
                    <p className='text-start text-gray-500 mb-[7.25rem]'>
                      ...
                      showcases the diverse range of industries we&apos;ve
                      worked with and the exceptional results we&apos;ve
                      achieved.
                    </p>
                  </div>
                </CarouselItem>
            ...
            <CarouselPrevious
              className={`absolute transition-colors duration-300 left-[35rem] top-36 rounded-md h-12 w-12 ${
                preActive && "!bg-blue-900 !text-gray-50"
              }`}
              onClick={() => {
                setNextActive(false);
                setPreActive(true);
            ...
            >
              <svg
                className={`size-4 pl-1 ${
                  preActive ? "fill-gray-50" : "fill-blue-900"
                }`}
            ...
            <CarouselNext
              className={`absolute transition-colors duration-300 right-3 top-36 rounded-md h-12 w-12 ${
                nextActive && "!bg-blue-900 !text-gray-50"
              }`}
              onClick={() => {
                setNextActive(true);
                setPreActive(false);
            ...
            >
              <svg
                className={`size-4 pl-1 ${
                  nextActive ? "fill-gray-50" : "fill-blue-900"
                }`}
                xmlns='http://www.w3.org/2000/svg'
              >
      ...
      <div className='grid grid-cols-4 gap-x-4 gap-y-8'>
        {CustomerQuotes.map(
          ({ name, position, description, company, image }, index) => (
            <Card
              className='bg-gray-200 border shadow-sm rounded-md transition-all duration-500 relative'
              key={index}
            >
              <CardHeader>
                ...
                <CardDescription className='flex flex-col gap-2'>
                  <h3 className=' font-normal text-lg text-gray-700 text-left'>
                    {name}
                  </h3>
                  <h3 className='font-normal text-lg text-blue-900 text-left'>
                    {position}, {company}
                  </h3>
                </CardDescription>
              </CardHeader>
